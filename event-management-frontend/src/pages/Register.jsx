  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      if (token) {
        ...
        return navigate(userRole === "customer" ? "/enquiry" : "/dashboard");
      }
      navigate("/login");
    } catch {
      ...
      <div className="w-full max-w-sm">
        <div className="bg-white p-6 shadow-2xl border border-[#e8e1d5]">
          ...
          <div className="mt-5 text-xs text-center text-black/70">
            Already a Partner?{" "}
            <Link to="/login" className="underline font-bold text-black hover:text-[#d4af37] transition-colors">
