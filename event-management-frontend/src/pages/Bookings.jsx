...
function BookingForm() {
  ...
  const handleSubmit = async (event) => {
    ...
    const notes = [
      ...
      form.categoryKey ? `Category Key: ${form.categoryKey}` : '',
      form.bookingType ? `Booking Type: ${form.bookingType}` : '',
      form.notes,
    ]
      .filter(Boolean)
      .join("\n");
...
export default function NewBookingPage() {
  return (
    <Suspense fallback={<div className="min-h-screen bg-[#f7f3ed] flex items-center justify-center"><Loader2 className="animate-spin text-[#b4975a]" size={34} /></div>}>
      <BookingForm />
    </Suspense>
  );
}
