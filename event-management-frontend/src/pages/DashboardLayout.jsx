...
export default function DashboardLayout() {
  ...
  const router = { replace: (url) => navigate(url, { replace: true }), push: (url) => navigate(url) };
    ...
    <div className="flex min-h-screen bg-[#f7f3ed]">
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 flex w-56 flex-col border-r-[2.5px] border-[#c4b096] bg-gradient-to-b from-[#F9F7F2] to-[#f0ebe3] transition-transform duration-300 lg:translate-x-0 ${
        ...
        <div className="flex items-center justify-between border-b-[2.5px] border-[#c4b096] px-4 py-3">
          <Link to="/dashboard" className="flex items-center gap-2 transition-all duration-300 hover:opacity-80">
            <img src="/logo.svg" alt="" className="h-8 w-auto" />
            <div className="text-sm font-extrabold text-[#3d2c1f] leading-tight">EventPro</div>
          </Link>
          <Link to="/" className="flex h-7 w-7 items-center justify-center rounded-lg border-[2.5px] border-[#c4b096] bg-white/60 text-[#8b7355] transition hover:border-[#C5A059] hover:text-[#C5A059]" title="Back to home">
                      ...
                      {isOpen && (
                        <div className="ml-3 mt-0.5 space-y-0.5 border-l-2 border-[#c4b096]/30 pl-1.5">
                          {item.subItems.map((sub) => {
                            ...
                            return (
                              <Link key={sub.href} to={sub.href} onClick={() => setSidebarOpen(false)}
      ...
      <div className="flex flex-1 flex-col min-w-0 lg:ml-56">
        <header className="sticky top-0 z-30 border-b-[2.5px] border-[#c4b096] bg-[#F9F7F2]/95 backdrop-blur-md">
          <div className="flex items-center justify-between px-4 py-3 lg:px-6">
            ...
            <Link to="/dashboard" className="items-center gap-2 transition-all duration-300 hover:opacity-80 flex">
